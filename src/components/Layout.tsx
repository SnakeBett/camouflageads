...
import {
  ...
  Shield, GraduationCap, LogOut, Menu, X, User, Diamond, Gauge,
} from "lucide-react";
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-card border-r border-border transform transition-transform duration-200 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
            {navItems.map((item) => (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${isActive(item.path) ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}
              >
                <item.icon className="h-4 w-4" />
                <span>{item.label}</span>
                {item.badge && <span className="ml-auto text-[10px] bg-muted px-2 py-0.5 rounded-full uppercase">{item.badge}</span>}
              </Link>
            ))}
          </nav>
          ...
          <div className="p-3 border-t border-border space-y-2">
            ...
            {isAdmin && (
              <Link to="/admin" className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-orange-400 hover:bg-orange-500/10">
                <Shield className="h-4 w-4" />Admin
              </Link>
            )}
            <button onClick={signOut} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-destructive hover:bg-destructive/10 w-full">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-30 bg-card border-t border-border flex lg:hidden">
        {bottomNav.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={`flex-1 flex flex-col items-center gap-0.5 py-2 text-[10px] transition-colors ${isActive(item.path) ? "text-foreground" : "text-muted-foreground"}`}
          >
